  ...
  >
    ...
    <div v-if="pagination" class="c-swipe-pagination">
      <div class="c-swipe-pagination-bar">
        <i v-for="item in length" :class="['c-swipe-pagination-item', item - 1 === insideValue ? 'active': '']" :key="item">Title</i>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    initDatas() {
      ...
      this.pages = this.$slots.default
        .filter(vm => vm.elm.classList&&vm.elm.classList.contains('c-swipe-item'))
        .map(vm => vm.elm);
