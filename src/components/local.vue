<template>
  <div class="articles" ref="articles">
    <scroll :data="songs" class="toplist" ref="toplist" :listenScroll="true" @scroll="onScroll" :pullDownConfig="pullDownConfig" @pullingDown="onPullingDown">
      ...
      <ul>
        <li @click="selectItem(song,index)" class="item" v-if="song.total&&song.total==song.downloaded" :class="{selected:song.id===selected}" v-for="(song,index) in songs">
          <div class="icon">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      selectItem(item,index) {
        ...
        this.setTopList(item)*/
          ...
          this.selectPlay({
            list: this.songs.filter(song=>song.total&&song.total==song.downloaded),
            index
          })
    ...
    computed:{
      ...mapGetters(['downloadAll','topList']),
      songs(){
        return this.allSongs.filter(x=>x.total&&x.total===x.downloaded)
      }
